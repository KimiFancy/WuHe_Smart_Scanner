    ...
    if (KEY_STATE_PRESSED == key_event->state) {
        if (hid_keyboard_get_char(key_event->modifier,
            ...
            if (key_char == KEYBOARD_ENTER_MAIN_CHAR) {
                ...
                if (barcode_len > 0) {
                    wuhe_ui_set_barcode_safe(barcode_buf);
                    wuhe_cloud_submit_scan(barcode_buf);
                }
                barcode_len = 0;
            } else if (key_char >= 0x20 &&
                       barcode_len < sizeof(barcode_buf) - 1) {
                barcode_buf[barcode_len++] = (char)key_char;
            }
